...
per_capita %>%
  ...
  mutate(alpha_2_code = forcats::fct_reorder(alpha_2_code, .x = n, max, .desc = TRUE),
         highlight = ifelse(alpha_2_code  == "NZ", TRUE, FALSE)) %>%
  ggplot(., aes(x = alpha_2_code, y = percapita, fill = highlight)) +
  geom_col() +
  theme_bw()
```
...
per_capita %>%
  ...
  ggplot(., aes(x = alpha_2_code, y = percapita, fill = nz_best)) +
  geom_col() +
  theme_bw() +
  labs(title = "NZ has best per capita rate of workshops")
```
...
country_join <- left_join(map.world, per_cap_world, by = c("region" = "entity")) %>%
