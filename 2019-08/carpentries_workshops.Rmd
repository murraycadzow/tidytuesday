...
```{r}
per_capita %>% arrange(desc(n)) %>% slice(1:20)%>% mutate(alpha_2_code = forcats::fct_reorder(alpha_2_code, .x = n, max, .desc = TRUE), highlight = ifelse(alpha_2_code  == "NZ", TRUE, FALSE)) %>% ggplot(., aes(x = alpha_2_code, y = n, fill = highlight)) + geom_col() + theme_bw()
```
...
```{r}
per_capita %>% arrange(desc(perCapita)) %>% slice(1:20)%>% mutate(alpha_2_code = forcats::fct_reorder(alpha_2_code, .x = perCapita, mean, .desc = TRUE), nz_best = ifelse(alpha_2_code  == "NZ", TRUE, FALSE)) %>% ggplot(., aes(x = alpha_2_code, y = perCapita, fill = nz_best)) + geom_col() + theme_bw() + labs(title = "NZ has best per capitia rate of workshops")
```
...
```{r}
per_capita %>% mutate(prop_workshops = n/sum(n)) %>% arrange(desc(prop_workshops)) %>% slice(1:20)%>% mutate(alpha_2_code = forcats::fct_reorder(alpha_2_code, .x = prop_workshops, max, .desc = TRUE), NZ = ifelse(alpha_2_code  == "NZ", TRUE, FALSE)) %>% ggplot(., aes(x = alpha_2_code, y = prop_workshops, fill = NZ)) + geom_col() + theme_bw() + labs(title = "Proportion of workshops taught by country")
