...
```{r}
per_capita %>% arrange(desc(perCapita)) %>% slice(1:20)%>% mutate(alpha_2_code = forcats::fct_reorder(alpha_2_code, .x = n, mean), highlight = ifelse(alpha_2_code  == "NZ", TRUE, FALSE)) %>% ggplot(., aes(x = alpha_2_code, y = perCapita, fill = highlight)) + geom_col() + theme_bw()
```
...
ggplot(data = country_join, aes(x = long, y = lat, group = group)) +
  ...
  scale_fill_continuous(high = "yellow", low = "blue", breaks = c(0,seq( 1e-7, 1e-5, length.out = 7  ))) +
  labs(title = "",
       subtitle = "",
       x = "",
       y = "",
