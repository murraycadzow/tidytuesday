...
```{r}
per_capita %>% arrange(desc(n)) %>% slice(1:20)%>% mutate(alpha_2_code = forcats::fct_reorder(alpha_2_code, .x = n, mean), highlight = ifelse(alpha_2_code  == "NZ", TRUE, FALSE)) %>% ggplot(., aes(x = alpha_2_code, y = perCapita, fill = highlight)) + geom_col() + theme_bw()
```
